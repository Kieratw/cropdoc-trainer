...
class MemmapPack(Dataset):
    ...
    def __getitem__(self, idx):
        # zawsze skala [0,1]; normalizacja tylko jeśli normalize=True
...
def load_dataloaders(dataset_root: Path, batch_size: int, workers: int, balance: bool):
    """
    Zwraca:
      - tr_dl (MemmapPack train, normalize=False)
      - va_dl (MemmapPack val, normalize=True)
      - classes, img_size, mean, std
    """
    ...
    # train: bez normalizacji, bo robimy ją w transformach v2
    ds_train = MemmapPack(train_pack, normalize=False)
    # val: normalizacja po staremu, bez augmentacji
    ...
    return tr, va, classes, img_size, mean, std
...
# -------------------- augmentacje v2 --------------------
def get_train_transforms(img_size: int, mean: np.ndarray, std: np.ndarray):
    """
    Augmentacje na CPU, wejście: batch [B,3,H,W] w [0,1].
    Ostatni krok: Normalize(mean,std).
    """
    ...
    return v2.Compose([
        # zoom & różne kadry
        v2.RandomResizedCrop(size=(img_size, img_size), scale=(0.6, 1.0), antialias=True),
        # trochę inna orientacja telefonu
        v2.RandomHorizontalFlip(p=0.5),
        v2.RandomVerticalFlip(p=0.1),
        v2.RandomRotation(degrees=20),
        # różne oświetlenie
        v2.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2),
        # upewniamy się co do typu
        v2.ToDtype(torch.float32, scale=False),
        # i dopiero teraz normalizacja pod model
...
def train_loop(args):
    ...
    tr_dl, va_dl, classes, img_size, mean, std = load_dataloaders(
        Path(args.data), args.batch_size, args.workers, args.balance
    )
    ...
    print(
        f"Model: {args.mode} | arch={arch_now} | params={count_params(model)/1e6:.2f}M | "
        f"classes={num_classes} | img={img_size}"
    )
    ...
    def make_opt(backbone_lr_scale: float):
        ...
        if back_trainable:
            groups.append(
                {
                    "params": back_trainable,
                    "lr": args.lr * backbone_lr_scale,
                    "base_lr": args.lr * backbone_lr_scale,
                }
            )
        if head_trainable:
            groups.append(
    ...
    # augmentacje v2 na CPU (tylko dla train)
    train_aug = get_train_transforms(img_size, mean, std) if args.aug else None
        ...
        for xb, yb in pbar:
            ...
            with torch.amp.autocast("cuda", enabled=args.amp):
                ...
                if teacher is not None:
                    alpha_now = min(
                        args.kd_alpha, args.kd_alpha * max(0, epoch - 1) / max(1, args.kd_warmup_epochs)
                    )
            ...
            pbar.set_postfix(
                lr=f"{lr_now:.1e}", loss=f"{loss.item():.4f}", kd_a=f"{alpha_now:.2f}", ls=f"{ls_now:.3f}"
            )
...
def parse_args():
    ...
    ap.add_argument("--balance", action="store_true", default=False, help="WeightedRandomSampler na train")
    ap.add_argument("--aug", action="store_true", help="Włącz augmentacje v2 na train")
