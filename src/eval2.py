...
def try_autodetect_meta(images_dir: Path) -> Optional[Path]:
    # .../<dataset_root>/test_images/cls  -> meta w <dataset_root>/cls/train/s000/meta.json
    try:
...
def infer_has_labels(img_paths: List[Path], class_names: List[str]) -> bool:
    # Label = nazwa folderu rodzica; musi być w klasach z ckpt
    for p in img_paths:
...
class FolderDataset(Dataset):
    def __init__(self, img_paths: List[Path], class_to_idx: Dict[str, int], img_size: int,
                 mean: np.ndarray, std: np.ndarray, has_labels: bool):
...
# ----------------- eval -----------------
@torch.no_grad()
def run_eval(ckpt_path: Path, images_dir: Path, meta_path_cli: Optional[Path],
             batch: int, workers: int, device: str = "cuda"):
    # 1) Ckpt
    ...
    save_dir = ckpt_path.parent / "eval"
    ...
    if has_labels:
        ...
        plot_confusion(cm, classes, cm_png, normalize=False, title="Confusion matrix (counts)")
        plot_confusion(cm, classes, cmn_png, normalize=True,  title="Confusion matrix (row-normalized)")
        print(f"Zapisano: {cm_png}")
        print(f"Zapisano: {cmn_png}")
        ...
        print("[ztest] Brak etykiet w folderze. CM i metryki pomijam, zapisuję same predykcje.")
    ...
    with open(preds_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for path, pred, prob_row in zip(paths_all, preds_all, probs_all):
            ...
            w.writerow([path, int(pred), pred_name, f"{conf:.6f}"] + [f"{float(p):.6f}" for p in prob_row])
    print(f"Zapisano: {preds_csv}")


def parse_args():
    ap = argparse.ArgumentParser("Auto-eval na folderze obrazów (auto-zapis wyników)")
    ap.add_argument("--ckpt", required=True, help="Ścieżka do best.pt")
    ap.add_argument("--images", required=True, help="Folder z obrazami (images/<klasa>/*.jpg lub images/*.jpg)")
    ap.add_argument("--meta", default=None, help="Opcjonalny meta.json (nadpisuje autodetekcję)")
