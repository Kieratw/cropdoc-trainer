...
class MobileNetV3Dual(nn.Module):
    ...
    Dropout w head_bin włączany w czasie treningu.
    """
    def __init__(self, num_classes: int, pretrained: bool = False, healthy_idx: int = 0):
        ...
        self.dropout_bin = nn.Dropout(0.2)  # Dropout w head_bin
    ...
    def forward(self, x) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        x = self.bn(x)
        x = self.relu(x)
        x_bin = self.dropout_bin(x) if self.training else x  # Dropout tylko w train
        return self.head_bin(x_bin), self.head_cls(x)
...
def build_model_from_ckpt(ck):
    ...
    m.load_state_dict(sd, strict=False)  # <- allow mismatch (Dropout w head_bin)
    m.eval()
    return m
...
def main():
    ...
    ap.add_argument("--image", help="Ścieżka do obrazu (RGB)")
    ap.add_argument("--bin_threshold", type=float, default=0.60)
    ap.add_argument("--cls_threshold", type=float, default=0.55)
    args = ap.parse_args()
    ...
    if not img_path:
        try:
            ...
            img_path = filedialog.askopenfilename(title="Wybierz obraz",
                filetypes=[("Obrazy","*.jpg;*.jpeg;*.png;*.bmp;*.webp"),("Wszystkie pliki","*.*")])
        except Exception:
            raise SystemExit("Podaj --image albo zainstaluj tkinter.")
    ...
    decision, label = decide(pb, pc, classes, healthy_idx, args.bin_threshold, args.cls_threshold)
